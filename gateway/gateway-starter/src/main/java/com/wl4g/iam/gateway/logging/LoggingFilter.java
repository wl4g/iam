...
public class LoggingFilter implements GlobalFilter, Ordered {
    ...
    private static final List<String> logHttpHeaders = unmodifiableList(new ArrayList<String>() {
    ...
    private Mono<Void> /* ServerHttpRequest */ logRequest(
        ...
        if (log5_10) {
            headers.forEach((headerName, headerValue) -> {
                if (log6_10 || logHttpHeaders.stream().anyMatch(h -> containsIgnoreCase(h, headerName))) {
                    requestLog.append("{}: {}\n");
    ...
    private ServerHttpResponse logResponse(
        ...
        if (log6_10) {
            ...
            httpHeaders.forEach((headerName, headerValue) -> {
                if (log8_10 || logHttpHeaders.stream().anyMatch(h -> containsIgnoreCase(h, headerName))) {
                    responseLog.append("{}: {}\n");
