...
public final class ReferenceCountedOpenSslServerContext extends ReferenceCountedOpenSslContext {
    ...
    private static final boolean ENABLE_SESSION_TICKET = SystemPropertyUtil
            .getBoolean("jdk.tls.server.enableSessionTicketExtension", false);

    ReferenceCountedOpenSslServerContext(X509Certificate[] trustCertCollection, TrustManagerFactory trustManagerFactory,
            X509Certificate[] keyCertChain, PrivateKey key, String keyPassword, KeyManagerFactory keyManagerFactory,
            Iterable<String> ciphers, CipherSuiteFilter cipherFilter, ApplicationProtocolConfig apn, long sessionCacheSize,
            long sessionTimeout, ClientAuth clientAuth, String[] protocols, boolean startTls, boolean enableOcsp, String keyStore,
        ...
        this(trustCertCollection, trustManagerFactory, keyCertChain, key, keyPassword, keyManagerFactory, ciphers, cipherFilter,
                toNegotiator(apn), sessionCacheSize, sessionTimeout, clientAuth, protocols, startTls, enableOcsp, keyStore,
                options);
    }
    ...
    ReferenceCountedOpenSslServerContext(X509Certificate[] trustCertCollection, TrustManagerFactory trustManagerFactory,
        ...
        super(ciphers, cipherFilter, apn, sessionCacheSize, sessionTimeout, SSL.SSL_MODE_SERVER, keyCertChain, clientAuth,
                protocols, startTls, enableOcsp, true, options);
        // Create a new SSL_CTX and configure it.
        boolean success = false;
        try {
            sessionContext = newSessionContext(this, ctx, engineMap, trustCertCollection, trustManagerFactory, keyCertChain, key,
                    keyPassword, keyManagerFactory, keyStore);
        ...
        try {
            try {
                ...
                if (!OpenSsl.useKeyManagerFactory()) {
                    ...
                    // keyManagerFactory for the server so build one if it is
                    // not specified.
                    if (keyManagerFactory == null) {
                    ...
                    SSLContext.setCertificateCallback(ctx,
                            new OpenSslServerCertificateCallback(engineMap, new OpenSslKeyMaterialManager(keyMaterialProvider)));
                }
            } catch (Exception e) {
            ...
            try {
                if (trustCertCollection != null) {
                    ...
                    // Mimic the way SSLContext.getInstance(KeyManager[], null,
                    // null) works
                ...
                if (PlatformDependent.javaVersion() >= 8) {
                    // Only do on Java8+ as SNIMatcher is not supported in
                    // earlier releases.
                    // IMPORTANT: The callbacks set for hostname matching must
                    // be static to prevent memory leak as
    ...
    private static final class OpenSslServerCertificateCallback implements CertificateCallback {
        ...
        public void handle(long ssl, byte[] keyTypeBytes, byte[][] asn1DerEncodedPrincipals) throws Exception {
            ...
            try {
                // For now we just ignore the asn1DerEncodedPrincipals as this
                // is kind of inline with what the
    ...
    private static final class OpenSslSniHostnameMatcher implements SniHostNameMatcher {
        ...
        public boolean match(long ssl, String hostname) {
            ...
            if (engine != null) {
                // TODO: In the next release of tcnative we should pass the
                // byte[] directly in and not use a String.
