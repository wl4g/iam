...
public final class SslProvider {
     ...
     * 
     * @param b
     *            a given bootstrap to enrich
     *
     * @return an enriched bootstrap
     */
    public static Bootstrap setBootstrap(Bootstrap b, SslProvider sslProvider) {
        BootstrapHandlers.updateConfiguration(b, NettyPipeline.SslHandler, new DeferredSslSupport(sslProvider));
     ...
     *
     * @param b
     *            a bootstrap to search and remove
     *
    ...
    public interface Builder {
         ...
         * {@link SslHandler}
         *
         * @param handlerConfigurator
         *            A callback given the generated {@link SslHandler}
         *
         * @return {@literal this}
         */
        ...
        /**
         * Set the options to use for configuring SSL handshake timeout. Default
         * to 10000 ms.
         *
        ...
        /**
         * Set the options to use for configuring SSL close_notify flush
         * timeout. Default to 3000 ms.
         *
         * @param closeNotifyFlushTimeout
         *            The timeout {@link Duration}
         *
         * @return {@literal this}
         */
         ...
         * timeout. Default to 3000 ms.
         *
         * @param closeNotifyFlushTimeoutMillis
         *            The timeout in milliseconds
         *
         * @return {@literal this}
         */
         ...
         * Default to 0 ms.
         *
         * @param closeNotifyReadTimeoutMillis
         *            The timeout in milliseconds
         *
         * @return {@literal this}
         */
    ...
    public enum DefaultConfigurationType {
         ...
         * {@link Http2SecurityUtil#CIPHERS}, ALPN support, HTTP/1.1 and HTTP/2
         * support
         *
         */
        H2
    ...
    void updateDefaultConfiguration() {
        ...
        case H2:
            sslContextBuilder
                    //
                    // Disabled for must using OPENSSL_REFINT.
                    //
                    // .sslProvider(io.netty.handler.ssl.SslProvider.isAlpnSupported(io.netty.handler.ssl.SslProvider.OPENSSL)
                    // ? io.netty.handler.ssl.SslProvider.OPENSSL_REFCNT
                    // : io.netty.handler.ssl.SslProvider.JDK)

                    //
                    // [Begin] ADD for must using OPENSSL_REFINT.
                    //
                    .sslProvider(io.netty.handler.ssl.SslProvider.isAlpnSupported(io.netty.handler.ssl.SslProvider.OPENSSL)
                            ? io.netty.handler.ssl.SslProvider.OPENSSL_REFCNT
                            : io.netty.handler.ssl.SslProvider.JDK)
                    //
                    // [End] ADD for must using OPENSSL_REFINT.
                    //
        ...
        case TCP:
            sslContextBuilder.sslProvider(
                    OpenSsl.isAvailable() ? io.netty.handler.ssl.SslProvider.OPENSSL : io.netty.handler.ssl.SslProvider.JDK);
            break;
        case NONE:
    ...
    public String asDetailedString() {
        return "handshakeTimeoutMillis=" + this.handshakeTimeoutMillis + ", closeNotifyFlushTimeoutMillis="
                + this.closeNotifyFlushTimeoutMillis + ", closeNotifyReadTimeoutMillis=" + this.closeNotifyReadTimeoutMillis;
    }

    @Override
    public String toString() {
        return "SslProvider {" + "type=" + type + ", handshakeTimeoutMillis=" + handshakeTimeoutMillis
                + ", closeNotifyFlushTimeoutMillis=" + closeNotifyFlushTimeoutMillis + ", closeNotifyReadTimeoutMillis="
    ...
    static final class Build implements SslContextSpec, DefaultConfigurationSpec, Builder {
         ...
         */
        static final long DEFAULT_SSL_HANDSHAKE_TIMEOUT = Long
                .parseLong(System.getProperty(ReactorNetty.SSL_HANDSHAKE_TIMEOUT, "10000"));
        ...
        public final Builder closeNotifyFlushTimeoutMillis(long closeNotifyFlushTimeoutMillis) {
            if (closeNotifyFlushTimeoutMillis < 0L) {
                throw new IllegalArgumentException(
                        "ssl close_notify flush timeout must be positive," + " was: " + closeNotifyFlushTimeoutMillis);
            }
        ...
        public final Builder closeNotifyReadTimeoutMillis(long closeNotifyReadTimeoutMillis) {
            if (closeNotifyReadTimeoutMillis < 0L) {
                throw new IllegalArgumentException(
                        "ssl close_notify read timeout must be positive," + " was: " + closeNotifyReadTimeoutMillis);
            }
        ...
        public boolean equals(Object o) {
            ...
            return handshakeTimeoutMillis == build.handshakeTimeoutMillis
                    ...
                    && Objects.equals(sslCtxBuilder, build.sslCtxBuilder) && type == build.type
        ...
        @Override
        public int hashCode() {
            return Objects.hash(sslCtxBuilder, type, sslContext, handlerConfigurator, handshakeTimeoutMillis,
                    closeNotifyFlushTimeoutMillis, closeNotifyReadTimeoutMillis);
        }
    }
    ...
    public static ServerBootstrap setBootstrap(ServerBootstrap b, SslProvider sslProvider) {
        ...
        BootstrapHandlers.updateConfiguration(b, NettyPipeline.SslHandler, new SslSupportConsumer(sslProvider, null));
    ...
    static final class SslSupportConsumer implements BiConsumer<ConnectionObserver, Channel> {
        final SslProvider sslProvider;
        ...
        public void accept(ConnectionObserver listener, Channel channel) {
            ...
            if (sniInfo != null) {
                sslHandler = sslProvider.getSslContext().newHandler(channel.alloc(), sniInfo.getHostString(), sniInfo.getPort());
                ...
                if (log.isDebugEnabled()) {
                    log.debug(format(channel, "SSL enabled using engine {}"), sslHandler.engine().getClass().getSimpleName());
                }
            }
            ...
            if (channel.pipeline().get(NettyPipeline.ProxyHandler) != null) {
                channel.pipeline().addAfter(NettyPipeline.ProxyHandler, NettyPipeline.SslHandler, sslHandler);
            } else if (channel.pipeline().get(NettyPipeline.ProxyProtocolReader) != null) {
                channel.pipeline().addAfter(NettyPipeline.ProxyProtocolReader, NettyPipeline.SslHandler, sslHandler);
            } else {
        ...
        public void userEventTriggered(ChannelHandlerContext ctx, Object evt) {
            if (evt instanceof SslHandshakeCompletionEvent) {
                ...
                if (handshake.isSuccess()) {
                    if (recorder != null) {
                        recorder.recordTlsHandshakeTime(ctx.channel().remoteAddress(),
                                Duration.ofNanos(System.nanoTime() - tlsHandshakeTimeStart), SUCCESS);
                    }
                    ctx.fireChannelActive();
                } else {
                    if (recorder != null) {
                        recorder.recordTlsHandshakeTime(ctx.channel().remoteAddress(),
                                Duration.ofNanos(System.nanoTime() - tlsHandshakeTimeStart), ERROR);
                    }
