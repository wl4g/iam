...
public class UserServiceImpl implements UserService {
	...
	public PageModel list(PageModel pm, String userName, String displayName) {
		IamPrincipal iamPrincipal = getPrincipalInfo();

		List<User> list = null;
		if (DEFAULT_SUPER_USER.equals(iamPrincipal.getPrincipal())) {
			pm.page(PageHelper.startPage(pm.getPageNum(), pm.getPageSize(), true));
			list = userDao.list(null, userName, displayName);
		} else {

			Set<Group> groups = groupService.getGroupsSet(new User(iamPrincipal.getPrincipal()));
